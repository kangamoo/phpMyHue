...
class HueEffect {
	...
	private function openEffect($effectName) {
		...
		if ($this->xr[$this->nodelvl]->open($filename)) {
			$ret = true;
		} else {
			echo $trs["Problem_for_opening_effect"] . " $filename.";
		}
	...
	private function processLoop() {
		...
		if ($this->debug) {
			echo str_repeat($this->indent, $this->nodelvl) . "&lt;<SPAN CLASS=effect-tag>end loop</SPAN>&gt;<BR>";
		}
	...
	private function processVar() {
		while ($this->xr[$this->nodelvl]->moveToNextAttribute()) {
			if (strtolower($this->xr[$this->nodelvl]->name) == "name") {
				$name = strtolower($this->getCurrentAttributeValue());
			} else {
				if (strtolower($this->xr[$this->nodelvl]->name) == "value") {
	...
	private function processLamp() {
		...
		while ($this->xr[$this->nodelvl]->moveToNextAttribute()) {
			$value = $this->getCurrentAttributeValue();
			if (strtolower($this->xr[$this->nodelvl]->name) == "id") {
				$id = $value;
			} else {
		...
		// Execute hue command
		if ($type == "light") {
			$action = $type . "s/$id/state";
		} else {
			$action = $type . "s/$id/action";
		}
	...
	private function processTimer() {
		...
		if ($this->debug) {
			echo str_repeat($this->indent, $this->nodelvl) . "<SPAN CLASS=effect-tag>timer</SPAN> = <SPAN CLASS=effect-value>$duration</SPAN> s<BR>";
		}
	...
	private function getColor() {
		...
		if ($lstate['on'] == "") {
			$this->color[$name]['on'] = "false";
		} else {
			$this->color[$name]['on'] = "true";
		}
